...
- **Self-healing via healCommand.** Verify failures can carry a \`healCommand\` re-run up to \`maxRetries\` on the working tree — exit-code gated (no LLM verifier). The \`healCommand\` is a static YAML string, not generated by the LLM. Git-worktree isolation for healing is not currently wired (\`src/worktree.ts\` exists but has zero production callers).
...
- **Self-healing via healCommand** — verify tasks with \`healCommand\` re-run a fix up to \`maxRetries\` on the working tree, exit-code gated (no LLM verifier).
...
Verify tasks can heal themselves — a failed verify re-runs \`healCommand\` up to \`maxRetries\` on the working tree, exit-code gated:
...
8. \`worktree.ts\` defines git worktree helpers (no production caller — healing does not use it); `recovery.ts` (v9) classifies failures and applies a deterministic heal policy.
