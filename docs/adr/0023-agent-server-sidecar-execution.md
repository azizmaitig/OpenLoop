...
Agent tasks are a distinct trust boundary: opt-in per task, governed by the existing L1/L2 human gates. The command denylist (`safety.ts`) does NOT see agent actions, so the denylist is passed into the prompt as instruction (soft). Hard enforcement arrives via `workspace.type: docker` for untrusted work.
